        ...
        try:
            ...
            for idx, data in enumerate(data_list):
                if data.shape != first_shape:
                    Logger.error(
                        "field {}: first_shape: {}, mismatched_shape: {}, mismatched_idx: {}".format(field, first_shape,
                                                                                                     data.shape, idx))
                    break
            raise e
    return episode_data
...
def submit_traj(data_server, step_data_list, last_step_data, rollout_desc, s_idx=None, e_idx=None,
                credit_reassign_cfg=None, assist_info=None):
    ...
    bootstrap_data = rename_fields(bootstrap_data, [EpisodeKey.NEXT_OBS, EpisodeKey.NEXT_STATE],
...
def rollout_func(
    ...
    while step <= rollout_length:
        ...
        policy_inputs = rename_fields(step_data, [EpisodeKey.NEXT_OBS, EpisodeKey.NEXT_STATE],
                                      [EpisodeKey.CUR_OBS, EpisodeKey.CUR_OBS])
        policy_outputs = {}

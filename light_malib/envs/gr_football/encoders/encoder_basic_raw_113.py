...
class FeatureEncoder:
    ...
    def observation_space(self):
        if self.pomdp:
            ...
            return Box(low=-1000, high=1000, shape=[(self.num_players-1)*7+70+(self.num_left+1)*2])
    ...
    def encode_each(self, state):
        ...
        zz_RewSap=np.concatenate((obs["left_team"][player_num].reshape(-1,2),obs_left_team,obs["ball"][:2].reshape(-1,2)))
        ...
        state_dict = {
            ...
            "avail": avail, # 19
            "zz_RewSap": zz_RewSap,#zz开头保证后面排序的时候在最后
